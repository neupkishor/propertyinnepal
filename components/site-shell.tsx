...
type SiteShellProps = {
  ...
  footerTools: readonly FooterTool[];
  navigation: readonly NavigationItem[];
};

export function SiteShell({ children, footerTools, navigation }: SiteShellProps) {
  ...
  return (
    <div className="relative isolate flex min-h-screen flex-col overflow-hidden">
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200 bg-white shadow-[0_12px_36px_rgba(15,23,42,0.18)]">
        <div
          className={`mx-auto flex max-w-[1440px] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:gap-6 lg:px-8 ${homepageRailPadding}`}
        >
      ...
      {isHomePage ? <FloatingSocialRail /> : null}

      <main className="flex-1 pt-[88px]">{children}</main>

      <footer className="border-t border-brand/20 bg-brand-deep text-white">
        <div
          className={`mx-auto grid max-w-[1440px] gap-10 px-6 py-12 md:grid-cols-2 lg:grid-cols-[1.25fr_0.65fr_0.65fr_0.9fr] lg:px-8 ${homepageRailPadding}`}
        >
          <div>
        ...
        <div
          className={`border-t border-white/15 px-6 py-4 text-center text-xs text-white/60 lg:px-8 ${homepageRailPadding}`}
        >
